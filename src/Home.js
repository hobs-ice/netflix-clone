...
if (showAdmin) return <Admin onBack={() => setShowAdmin(false)} />;
  ...
  <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'black', zIndex: 200, display: 'flex', flexDirection: 'column' }}>
    ...
    />
      ...
      {/* HERO */}
      <div style={{ height: '80vh', background: 'linear-gradient(to right, rgba(0,0,0,0.9) 40%, transparent), linear-gradient(to top, #141414 5%, transparent)', display: 'flex', alignItems: 'center', padding: '0 40px', paddingTop: 80 }}>
        {films[0] ? (
          <div>
  ...
  ▶ Lecture
      ...
      <div style={{ padding: '0 40px 40px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 16 }}>
          {films.map(film => (
            <div key={film.id} onClick={() => setCurrentFilm(film)} style={{ borderRadius: 4, overflow: 'hidden', cursor: 'pointer', transition: 'transform 0.2s' }}
  ...
  onMouseLeave={e => e.currentTarget.style.transform = 'scale(1)'}>
              ...
              {film.thumbnail_url ? (
                ...
                <div style={{ width: '100%', aspectRatio: '16/9', background: '#333', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 32 }}>🎬</div>
              )}
